...
export default function PulloutRequestsPage() {
    ...
    const [returnableSupplies, setReturnableSupplies] = useState<
        ReturnableSupply[]
    >([]);
    const [loading, setLoading] = useState(true);
    const [activeTab, setActiveTab] = useState<'requests' | 'returns'>(
        'requests',
    );
    ...
    const loadReturnableSupplies = async () => {
        try {
            const response = await axios.get(
                '/pullout-requests/returnable/list',
            );
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="flex flex-col gap-6 p-6">
                ...
                <div className="flex gap-2">
                    <Button
                        variant={
                            activeTab === 'requests' ? 'highlight' : 'outline'
                        }
                    ...
                    <Button
                        variant={
                            activeTab === 'returns' ? 'highlight' : 'outline'
                        }
                                ...
                                <Table>
                                    ...
                                    <TableBody>
                                        {returnableSupplies.map((supply) => (
                                            <TableRow
                                                key={
                                                    supply.pullout_request_details_id
                                                }
                                            >
                                                <TableCell>
                                                    #{supply.service_order_id} -{' '}
                                                    {supply.service_name}
                                                </TableCell>
                                                <TableCell>
                                                    {supply.employee_name}
                                                </TableCell>
                                                <TableCell>
                                                    {supply.supply_name}
                                                </TableCell>
